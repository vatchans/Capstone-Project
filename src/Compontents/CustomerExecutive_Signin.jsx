...
function CustomerExecutive_Signin() {
    ...
    let handlesubmit = async(e) => {
      ...
      setErrorMessage2(false)
        try{
        let res=await axios.post("https://milk-way-crm.onrender.com/CustomerExcecutive/Signin",
        {
            Email,
            Password
         ...
         if(res.status===200){
          ...
          await localStorage.setItem("CustomerExcecutive",JSON.stringify(res.data.token))
          navigate(-1)
         }
    ...
    {wrgemail?<div className="error mt-2">{showerr} <i class="fa-solid fa-circle-exclamation"></i></div>:<></>}    
       ...
       <Link to="/CustomerExecutive_resetPassword" style={{"textDecoration":"none","color":"black"}}><p className="m-5 mb-3 mt-3">Forget password <img src='icons8-forgot-password-100.png' width={"15px"}alt="?"></img></p></Link> 
        <Form.Group className="text-center w-35 mb-3" controlId="formBasicEmail">
            ...
            >
                ...
                <p  className="text-center mb-3 mt-3 info">Don't have an account.Register&nbsp;<Link to="/CustomerExecutive_Signup"><span>here</span></Link></p>
                </Form.Group>
            </Form>
            </div>
        </div>
...
export default CustomerExecutive_Signin;