...
const {Admin}=require('../public/dbconfig/Schema/Admin')
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    if (existinguser) {
      ...
      let existinguser = await Admin.updateMany({ Email: req.body.Email }, {
        $set: {
          Auth: auth
        }
      })
...
router.post('/Autenticate-code', async (req, res) => {
  try {
    let existinguser = await Admin.findOne({ Auth: req.body.code })
...
router.post("/new-password/:Email/", async (req, res) => {
  try {
    ...
    let newpassword = await Admin.updateOne({ Email: req.params.Email }, { $set: { Password:hash} })
    await Admin.updateOne({Email:req.params.Email},{$unset:{Auth:""}})
